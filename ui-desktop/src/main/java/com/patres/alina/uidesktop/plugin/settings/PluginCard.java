...
public class PluginCard extends CardItem {
    ...
    protected void updateState(UpdateStateRequest updateStateRequest) {
        getAlinaRestClient().updatePluginState(updateStateRequest);
    }

    @Override
    protected void deleteCard() {
        getAlinaRestClient().deletePlugin(cardListItem.id());
    }
    ...
    protected ApplicationModalPaneContent createEditPane() {
        final PluginDetail pluginDetails = getAlinaRestClient().getPluginDetails(cardListItem.id());
