...
import static com.patres.alina.uidesktop.backend.ErrorResponseReceiver.getErrorResponseMessageIfPossible;
...
public class ChatWindow extends BorderPane {
    ...
    public ChatWindow(ChatThreadResponse chatThread, ApplicationWindow applicationWindow) {
        ...
        messages = AlinaRestApi.getAlinaRestClient().getMessagesByThreadId(chatThread.id());
    ...
    private void stopAndSendRecording(File audio) {
        try {
            ...
            String message = AlinaRestApi.getAlinaRestClient().sendChatMessagesAsAudio(audio).content();
            displayMessage(message, ChatMessageRole.USER, ChatMessageStyleType.NONE);
            sendMessageToService(message);
        } catch (FeignException e) {
            final ChatMessageResponseModel chatMessageResponseModel = handelExceptionAsMessage("<speech>", e, getErrorResponseMessageIfPossible(e));
    ...
    private static ChatMessageResponseModel getChatResponse(final ChatMessageSendModel chatMessageSendModel) {
        try {
            return AlinaRestApi.getAlinaRestClient().sendChatMessages(chatMessageSendModel);
        } catch (FeignException e) {
            return handelExceptionAsMessage(chatMessageSendModel.content(), e, getErrorResponseMessageIfPossible(e));
        } catch (Exception e) {
