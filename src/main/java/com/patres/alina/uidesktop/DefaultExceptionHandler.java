...
public class DefaultExceptionHandler implements Thread.UncaughtExceptionHandler {
    ...
    public void uncaughtException(Thread t, Throwable e) {
        var dialog = createAlert(e);
        if (dialog != null) {
            dialog.showAndWait();
        }
    }

    private @Nullable Alert createAlert(Throwable e) {
        if (e instanceof WarningException) {
            return createWarningDialog(e);
        } else {
            e.printStackTrace();
            return createExceptionDialog(e);
        }
    }
    ...
    private Alert createWarningDialog(Throwable throwable) {
        Objects.requireNonNull(throwable);
